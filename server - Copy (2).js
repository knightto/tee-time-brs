/* server.js v3.9 date+email templates */
...
// Interpret date-only strings as UTC (avoid timezone drift)
...
// Store dates as "noon UTC" to be robust against later formatting in any TZ
...
const fmt = {
  ...
  dateLong(x){ const d = asUTCDate(x); return isNaN(d) ? '' : d.toLocaleDateString(undefined,
    { weekday:'long', month:'long', day:'numeric', year:'numeric', timeZone:'UTC' }); },
  dateShortTitle(x){ const d = asUTCDate(x); return isNaN(d) ? '' : d.toLocaleDateString(undefined,
...
function frame(title, body){
  return `
  <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;background:#f6f7f9;padding:24px">
    <tr><td align="center">
      <table role="presentation" cellpadding="0" cellspacing="0" width="600" style="max-width:600px;background:#ffffff;border-radius:10px;padding:24px;border:1px solid #e5e7eb">
        <tr><td>
          <h2 style="margin:0 0 12px 0;color:#111827;font-size:20px">${esc(title)}</h2>
          ${body}
          <p style="color:#6b7280;font-size:12px;margin-top:24px">You received this because you subscribed to tee time updates.</p>
        </td></tr>
      </table>
    </td></tr>
  </table>`;
...
function eventDeletedEmail(ev){
  const body = `
    <p style="margin:0 0 6px 0"><strong>Event:</strong> ${esc(fmt.dateShortTitle(ev.date))} — ${esc(ev.course||'')}</p>
    <p style="margin:0 0 6px 0">This event has been canceled or removed.</p>`;
  return frame('An Event Has Been Removed', body);
}
function teeAddedEmail(ev, time){
  const body = `
    <p style="margin:0 0 6px 0"><strong>Event:</strong> ${esc(fmt.dateShortTitle(ev.date))} — ${esc(ev.course||'')}</p>
    <p style="margin:0 0 6px 0"><strong>Tee Time:</strong> ${esc(fmt.tee(time))}</p>
    ${btn('Go to Sign-up Page')}`;
  return frame('A New Tee Time Was Added', body);
}
function teamAddedEmail(ev){
  const body = `
    <p style="margin:0 0 6px 0"><strong>Event:</strong> ${esc(fmt.dateShortTitle(ev.date))} — ${esc(ev.course||'')}</p>
    <p style="margin:0 0 6px 0">Another team slot is now available.</p>
    ${btn('Go to Sign-up Page')}`;
...
app.post('/api/events', async (req, res) => {
  try {
    ...
    const created = await Event.create({
      course,
      date: toNoonUTC(date),                 // store robustly
      notes,
    ...
    await sendEmailToAll(
      `New Event: ${created.course} (${fmt.dateISO(created.date)})`,
      eventCreatedEmail(created)
    );
  } catch (e) {
...
app.post('/api/events/:id/tee-times', async (req, res) => {
  ...
  if (ev.isTeamEvent) {
    ...
    return void sendEmailToAll(`Team Added: ${ev.course} (${fmt.dateISO(ev.date)})`, teamAddedEmail(ev));
  }
  const { time } = req.body || {};
...
app.delete('/api/events/:id/tee-times/:teeId', async (req, res) => {
  try {
    ...
    const html = isTeam ? teamAddedEmail(ev).replace('A New Team Was Added','A Team Was Removed').replace('Another team slot is now available.','A team was removed from this event.')
